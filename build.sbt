...
val scalaFixSettings: List[Def.Setting[_]] =
  List[Def.Setting[_]](scalafixScalaBinaryVersion := "2.13")

val generateCode = taskKey[Unit](
  "A task for generating the code starting from the swagger definition"
)

val packagePrefix =
  settingKey[String]("The package prefix derived from the uservice name")
...
lazy val serverGenerated = project
  .in(file("server-generated"))
  .settings(
    name := "datamesh.airbytespecificprovisioner.server.generated",
    scalacOptions := Seq()
  )
  .setupBuildInfo

lazy val clientGenerated = project
  .in(file("client-generated"))
  .settings(
    name := "datamesh.airbytespecificprovisioner.client",
    scalacOptions := Seq(),
    libraryDependencies := Dependencies.Jars.client,
    coverageEnabled := false,
    app.k8ty.sbt.gitlab.K8tyGitlabPlugin.gitlabProjectId := "40097537"
  )
...
lazy val root = (project in file("."))
  .settings(
    ...
    Docker / packageName :=
      s"registry.gitlab.com/agilefactory/witboost.mesh/provisioning/witboost.mesh.provisioning.airbytespecificprovisioner",
  ...
  .aggregate(clientGenerated)
  .dependsOn(serverGenerated)
